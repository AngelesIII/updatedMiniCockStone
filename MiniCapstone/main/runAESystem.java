...
public class runAESystem extends absMethods { //goal to make kinda reallistic by implementing loading using thread.sleep
    ...
    public void manageProducts() { 
        ...
        try {
            ...
            while(fetchProducts.next()) { 
                ...
                System.out.printf("\t\t\t\t\t\t\t   %1s%14s%15s%21s%15s" ,fetchProducts.getInt("product_id") ,fetchProducts.getString("category"), fetchProducts.getString("product_name"), fetchProducts.getString("product_brand"), fetchProducts.getInt("product_quantity"));
                System.out.println();
    ...
    public void viewCriticalLevel() { 
        try {
            ...
            System.out.println("\t\t\t\t ---------------------------------------------------------------------------------------------------------------------------");
            System.out.printf("\t\t\t\t\t   %8s%18s%18s%18s", " ID" ,"      PRODUCT NAME" , "   PRODUCT BRAND", " QUANTITY");
            System.out.println();
            System.out.println("\t\t\t\t ---------------------------------------------------------------------------------------------------------------------------");

            while(criticalLevelQuery.next()) { //while it has a row to be fetch, display the row from the data base

                System.out.printf("\t\t\t\t\t   %8s%15s%18s%21s" ,criticalLevelQuery.getInt("product_id") ,criticalLevelQuery.getString("product_name"), criticalLevelQuery.getString("product_brand"), criticalLevelQuery.getInt("product_quantity"));
                System.out.println();
    ...
    public void viewExpiredItems() { //function for viewing the expired products table.
        ...
        try {
            ...
            System.out.println("\t\t\t\t      -----------------------------------------------------------------------------------------------------------");
            System.out.printf("\t\t\t\t\t        %20s%15s%15s%15s", " ID" ,"      PRODUCT CATEGORY"     , "PRODUCT NAME", "PRODUCT BRAND");
            System.out.println();
            System.out.println("\t\t\t\t      -----------------------------------------------------------------------------------------------------------");

            while(fetchExpiredProducts.next()) { 

                System.out.printf("\t\t\t\t\t        %20s%14s%20s%14s" ,fetchExpiredProducts.getInt("product_id") ,fetchExpiredProducts.getString("product_category"), fetchExpiredProducts.getString("product_name"), fetchExpiredProducts.getString("product_brand"));
                System.out.println();
